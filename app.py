...
def process_local_file(file_path):
    try:
        ...
        else: date_obj = date.today() # 簡化日期判斷
...
# ============================================
# 2. 演算法 (移除 DayClose 依賴)
# ============================================
def calculate_hurst(ts): # 移除 Hurst 因為需要股價時間序列
    return 0.5 
...
def view_ai_strategy():
    ...
    # [V66] 移除 Hurst 與 蒙地卡羅 (因為這些嚴重依賴正確的收盤價，而您的資料無收盤價)
    st.info("⚠️ 由於籌碼資料不包含正確收盤價，Hurst 指數與蒙地卡羅模擬暫停使用，以避免誤導。")
    st.markdown("---")
    
    # NLP Sentiment (依賴成交量，可保留)
...
def view_trend_analysis():
    ...
    if target_brokers:
        ...
        st.dataframe(pd.DataFrame(stats).style.format("{:,.1f}", subset=['淨買賣(張)']).format("{:.2f}", subset=['均價']), use_container_width=True, hide_index=True)
        ...
        if not detail_show.empty:
            ...
            st.dataframe(detail_show.style.format("{:.1f}", subset=['買進(張)','賣出(張)','淨買賣(張)']).format("{:.2f}", subset=['買均']).background_gradient(subset=['淨買賣(張)'], cmap='RdYlGn'), use_container_width=True, hide_index=True)
...
def view_winners():
    ...
    # 既然沒有收盤價，我們改用「大量買進」作為贏家潛力股，「大量賣出」作為潛在賣壓
    ...
    with c1:
        ...
        st.dataframe(w_show.style.format("{:.1f}", subset=['淨買(張)']).format("{:.2f}", subset=['成本']).background_gradient(subset=['淨買(張)'], cmap='Reds'), use_container_width=True, hide_index=True)
    ...
    with c2:
        ...
        st.dataframe(l_show.style.format("{:.1f}", subset=['淨買(張)']).format("{:.2f}", subset=['成本']).background_gradient(subset=['淨買(張)'], cmap='Greens_r'), use_container_width=True, hide_index=True)

# ============================================
# 9. 視圖：🕵️‍♂️ 分點偵探 (移除股價線圖)
# ============================================
def view_broker_detective():
    ...
    if not data.empty:
        ...
        st.dataframe(show.style.format("{:.1f}", subset=['買進(張)','賣出(張)','淨買賣(張)']).format("{:.2f}", subset=['買均']).background_gradient(subset=['淨買賣(張)'], cmap='RdYlGn'), use_container_width=True, hide_index=True)
...
def view_batch_import():
    ...
    st.info("請將每日的 CSV 檔案拖曳至下方，系統將自動合併並更新資料庫。")
    uploaded_files = st.file_uploader("選擇 CSV 檔案 (支援多選)", type=['csv'], accept_multiple_files=True)
    
    if uploaded_files and st.button("🚀 確認匯入"):
        ...
        if all_dfs:
            with st.spinner("正在合併資料庫..."):
                final_df = pd.concat(all_dfs, ignore_index=True)
                save_to_db(final_df)
            st.success(f"🎉 成功匯入 {len(all_dfs)} 筆資料！請重新整理網頁查看最新數據。")
        else: st.error("❌ 檔案解析失敗，請確認格式。")
        
    st.markdown("---")
    st.subheader("🛠️ 資料庫維護")
    if st.button("🧹 執行深度清洗 (修復亂碼)"):
        with st.spinner("清洗中..."):
            if os.path.exists(CSV_FILE):
                df = pd.read_csv(CSV_FILE)
                if 'Broker' in df.columns:
...
def main():
    with st.sidebar:
        st.title("🦅 Phoenix V66")
        st.caption("網頁實戰版")
        st.markdown("---")
        # 修改最後一個選項名稱為「每日資料上傳/匯入」
